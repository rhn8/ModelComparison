export function skinDetection(imageData) {
    
    const data = imageData.data;

    // Define the bounds for skin detection in HSV and YCrCb
    const lowerBoundHSV = {h: 0, s: 10, v: 60};
    const upperBoundHSV = {h: 20, s: 170, v: 255};
    ...
    for (let i = 0; i < data.length; i += 4) {
        ...
        if (inRange(hsv.h, hsv.s, hsv.v, lowerBoundHSV, upperBoundHSV) ||
            inRangeYCrCb(ycrcb.y, ycrcb.cr, ycrcb.cb, lowerBoundYCrCb, upperBoundYCrCb)) {
            data[i+3] = 20; // Mark as skin pixel
        }
    }
